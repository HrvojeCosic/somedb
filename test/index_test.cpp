...
TEST_F(IndexTestFixture, CreateIndex_SerializeAndDeserializeNode) {
    ...
    LEAF_RECORDS(page->values).push_back(rid1);
    LEAF_RECORDS(page->values).push_back(rid2);
    LEAF_RECORDS(page->values).push_back(rid3);
...
TEST_F(IndexTestFixture, InsertTest_RootWithEnoughSpace) {
    ...
    ASSERT_EQ(LEAF_RECORDS(leaf->values).at(0).pid, val1.pid);
    ...
    ASSERT_EQ(LEAF_RECORDS(leaf->values).at(0).pid, val1.pid);
    ASSERT_EQ(LEAF_RECORDS(leaf->values).at(1).pid, val1.pid);
}
...
TEST_F(IndexTestFixture, InsertTest_FullRoot) {
    ...
    BTreeKey key2 = {reinterpret_cast<u8 *>(data2.data()), static_cast<u8>(data2.length())};
    BTreeKey key3 = {reinterpret_cast<u8 *>(data3.data()), static_cast<u8>(data3.length())};
    BTreeKey key4 = {reinterpret_cast<u8 *>(data4.data()), static_cast<u8>(data4.length())};
    BTreeKey key5 = {reinterpret_cast<u8 *>(data5.data()), static_cast<u8>(data5.length())};

    auto leaf_pid1 = tree->insert(key1, val1);
    tree->insert(key2, val2);
    tree->insert(key3, val3);
    auto leaf_pid = tree->insert(key4, val4);
    ASSERT_EQ(leaf_pid, leaf_pid1); // all kv pairs are in same node
    ...
    ASSERT_EQ(ok, false);
    ASSERT_EQ(curr_root->keys.size(), pre_size);
    ...
    ASSERT_EQ((char *)curr_root->keys.at(0).data == data5, true); // "C" in middle
    ASSERT_EQ(curr_root->keys.size(), 1);
    std::vector<BTreeKey> left_node_keys = {key1, key2};        // A B
    std::vector<BTreeKey> right_node_keys = {key5, key3, key4}; // C D E
    ASSERT_EQ(old_root->keys == left_node_keys, true);
    ASSERT_EQ(new_child->keys == right_node_keys, true);
}
