...
/*
 * Allocates memory for a new page of the given id and returns a pointer to it's beginning
 */
void *new_page(uint32_t id); 

/*
 * Returns a pointer to the beginning of the tuple with the given index at the specified page, 
 * or a null pointer if the tuple does not exist in the given page
 */
...
/*
 * Writes contents of the given page to file with the specified file descriptor fd
 */
void write_page(void* page, uint32_t fd);

/*
 * Reads contents of the page of specified page_id belonging to a file of specified file descriptor fd
 * into memory and returns a pointer to its beginning
 */
void *read_page(uint32_t page_id, uint32_t fd);

/*
 * Adds given tuple to the given page and returns a pointer to beginning of added tuple.
 * If the page is full, immediately returns a null pointer
 */
TuplePtr *add_tuple(void* page, void* tuple, uint16_t tuple_size);

/*
 * Returns information about tuple pointers at the specified page contained in TuplePtrList
 */
